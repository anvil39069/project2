...
//adding search feature to find the authors book in the list
void searchbyauthor() {
    system("cls");
    FILE *fp;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        // If the author name is found in the line, print it
...
void searchbygenre() {
    ...
    // Convert genre_name to uppercase for case-insensitive search
    ...
    // Search for the genre name in the file
    while (fgets(line, sizeof(line), fp)) {
        // Convert the line from the file to uppercase
        char line_upper[200];
        strcpy(line_upper, line);
        ...
        // If the genre_name is found in the line, print it
